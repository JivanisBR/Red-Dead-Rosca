...
int n6x = 0, n6y = 0;
float tamx=0, tamy=0;
float tamx2=0, tamy2=0;
float tamx3=0, tamy3=0;
float tamx4=0, tamy4=0;
float tamx5=0, tamy5=0;
float tamx6=0, tamy6=0;
int bot=0;
...
int main(void)
{
    ...
    n2x = GetRandomValue(200,300); n2y = GetRandomValue(0,100);
    n3x = GetRandomValue(300,400); n3y = GetRandomValue(50,100);
    n4x = GetRandomValue(400,500); n4y = GetRandomValue(0,50);
    n5x = GetRandomValue(-300,-100); n5y = GetRandomValue(0,50);
    n6x = GetRandomValue(-400,-300); n6y = GetRandomValue(50,100);
    tamx=GetRandomValue(100, 300)/3.0f; tamy=GetRandomValue(50, 100)/3.0f;
    tamx2=GetRandomValue(50, 100)/3.0f; tamy2=GetRandomValue(40, 70)/3.0f;
    tamx3=GetRandomValue(300, 500)/3.0f; tamy3=GetRandomValue(50, 100)/3.0f;
    tamx4=GetRandomValue(150, 700)/3.0f; tamy4=GetRandomValue(50, 100)/3.0f;
    tamx5=GetRandomValue(150, 200)/3.0f; tamy5=GetRandomValue(50, 150)/3.0f;
    tamx6=GetRandomValue(100, 300)/3.0f; tamy6=GetRandomValue(30, 80)/3.0f;
    ...
    if(pause==false && modo==false && menu==false && bangmenu==false && oneshotmenu==false && bangbang==true){
        ...
        if(timer_inicio<1 && colocarnomes==false){
            ...
            if(solo==false){
                ...
                if(IsKeyDown(KEY_I) && !pulando2 && cb2_y >= CHAO && pause==false){ PlaySound(pulo); emote2=false; contemt2=0; pulando2 = true; vel_y2 = -FORCA_PULO_INICIAL; }
            }
            else { 
            ...
            if(bangbang==true){
                ...
                if(espaco_atual && !espaco_pressionado && mun > 0 && cooldown1 == 0 && r1 == false && pause==false){
                    emote1=false;
                    for (int i = 0; i < MAX_BALAS; i++){
                        if(bala1_ativo[i] == 0){
                            bala1v_x[i] = cb_x + 53;
                ...
                if(solo == false) {
                    if(m_atual && !m_pressionado && mun2 > 0 && cooldown2 == 0 && r2 == false && pause==false){
...
void desenhar(){
    ...
    if(menu==true){
        ...
        if(creds==false){
            ...
            DrawEllipse(n3x, n3y, tamx3, tamy3, WHITE);
            DrawEllipse(n4x, n4y, tamx4, tamy4, WHITE);
            DrawEllipse(n5x, n5y, tamx5, tamy5, WHITE);
            DrawEllipse(n6x, n6y, tamx6, tamy6, WHITE);
    ...
    if(menu==false && modo==false && quit==false && bangmenu==false){
        ...
        DrawEllipse(n6x, n6y, tamx6, tamy6, WHITE);
        aux++;
        if(aux%50==0){ n1x++; n2x++; n3x++; n4x++; n5x++; n6x++; }
        ...
        if (oneshot == true) {
            ...
            if (os_estado == 3) {
                ...
                if (dead1 > 0 && dead2 > 0 && pause==false){ DrawEllipse(760, 400, 150, 50, WHITE); DrawEllipse(630, 420, 30, 10, WHITE); DrawTexture(neg, 400,340, WHITE); DrawTextEx(fnt,"aperta R pra reiniciar", {660, 390}, 20, 3.0, BLACK); }
        ...
        if(dead2>0 && dead1>0 && bangbang==true && pause==false){ 
            DrawEllipse(760, 400, 150, 50, WHITE); DrawEllipse(630, 420, 30, 10, WHITE); DrawTexture(neg, 400,340, WHITE); DrawTextEx(fnt,"aperta R pra reiniciar", {660, 390}, 20, 3.0, BLACK);
            dnv=1; 
            if(IsKeyPressed(KEY_R)){ 
